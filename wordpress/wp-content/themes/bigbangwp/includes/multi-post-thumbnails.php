...
if (!class_exists('MultiPostThumbnails')) {
	...
	class MultiPostThumbnails {
        ...
        public function enqueue_admin_scripts() {
            $template_url = get_bloginfo('template_url') . '/includes/multi-post-thumbnails-admin.js';
		...
		private function post_thumbnail_html($thumbnail_id = NULL) {
			...
			if ($thumbnail_id && get_post($thumbnail_id)) {
				...
				if (!empty($thumbnail_html)) {
                    ...
                    $caption = get_post_meta($post_ID, $this->post_type . '_' . $this->id . '_caption', true);
                    $description = get_post_meta($post_ID, $this->post_type . '_' . $this->id . '_description', true);  
                    $url = get_post_meta($post_ID, $this->post_type . '_' . $this->id . '_url', true);
                    
					$ajax_nonce = wp_create_nonce("set_post_thumbnail-{$this->post_type}-{$this->id}-{$post_ID}");
					$content = sprintf($set_thumbnail_link, $thumbnail_html);
                    $content .= 'Caption: <input type="text" autocomplete="off" size="" class="widefat" name="' . $this->post_type . '_' . $this->id . '_caption' . '" id="' . $this->post_type . '_' . $this->id . '_caption' . '" value="' . $caption.'">';
					$content .= 'Description: <input type="text" autocomplete="off" size="" class="widefat" name="' . $this->post_type . '_' . $this->id . '_description' . '" id="' . $this->post_type . '_' . $this->id . '_description' . '" value="' . $description .'">';
                    $content .= 'URL: <input type="text" autocomplete="off" size="" class="widefat" name="' . $this->post_type . '_' . $this->id . '_url' . '" id="' . $this->post_type . '_' . $this->id . '_url' . '" value="' . $url .'">'; 
		...
		public function set_thumbnail() {
			...
			if ($thumbnail_id == '-1') {
                ...
                //delete_post_meta($post_ID, "{$this->post_type}_{$this->id}_caption");
                //delete_post_meta($post_ID, "{$this->post_type}_{$this->id}_description");
                //delete_post_meta($post_ID, "{$this->post_type}_{$this->id}_url");
			...
			if ($thumbnail_id && get_post($thumbnail_id)) {
				...
				if (!empty($thumbnail_html)) {
                    $caption = get_post_meta($post->ID, "{$this->post_type}_{$this->id}_caption", true);
                    $description = get_post_meta($post->ID, "{$this->post_type}_{$this->id}_description", true);  
                    $url = get_post_meta($post->ID, "{$this->post_type}_{$this->id}_url", true);  
                    
                    update_post_meta($post_ID, "{$this->post_type}_{$this->id}_thumbnail_id", $thumbnail_id);
/*                    update_post_meta($post_ID, "{$this->post_type}_{$this->id}_caption", $caption);
                    update_post_meta($post_ID, "{$this->post_type}_{$this->id}_description", $description);
                    update_post_meta($post_ID, "{$this->post_type}_{$this->id}_url", $url); */
